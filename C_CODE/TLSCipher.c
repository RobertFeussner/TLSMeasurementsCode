...
int main(int argc, char* argv[]) 
{
	if(argc == 2 && ((strcmp("-h", argv[1]) == 0) || (strcmp("--help", argv[1]) == 0))){
		printf("\tThis function records the time for a connection to a given url for TLS 1.3 with a list of ciphers.\n");
		printf("\tInput parameters:\n\t\tURL\t- e.g. google.com\n\t\tIP version\t- 4 or 6\n");
		printf("\tOutput:\n");
		printf("\t\tWEBSITE;IP_VERSION;CIPHER;ERROR;ERROR_STRING;NAMELOOKUP_TIME;CONNECT_TIME;APPCONNECT_TIME;TOTAL_TIME\n");
		exit(EXIT_SUCCESS);
	}
	...
	for(int i = 0;i < NUMBER_OF_CIPHER; i++){
		...
		printf("%s;%d;%s;%d;%s;", url, ipVersion, ciphers[i], response, curl_easy_strerror(response));
