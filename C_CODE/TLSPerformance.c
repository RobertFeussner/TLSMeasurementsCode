...
int main(int argc, char* argv[]) 
{
	if(argc == 2 && ((strcmp("-h", argv[1]) == 0) || (strcmp("--help", argv[1]) == 0))){
		printf("\tThis function records the time for a connection to a given url for TLS 1.2 and TLS 1.3.\nThe IP version can be specified.\n");
		printf("\tInput parameters:\n\t\tURL\t- e.g. google.com\n\t\tIP version\t- 4 or 6\n");
		printf("\tOutput:\n");
		printf("\t\tWEBSITE;IP_VERSION;MEASUREMENT;TLS12_NAMELOOKUP_TIME;TLS12_CONNECT_TIME;TLS12_APPCONNECT_TIME;TLS12_TOTAL_TIME;TLS13_NAMELOOKUP_TIME;TLS13_CONNECT_TIME;TLS13_APPCONNECT_TIME;TLS13_TOTAL_TIME\n");
		exit(EXIT_SUCCESS);
	}
	...
	if(argc == 4){
		numberMeasurements = atoi(argv[3]);
	}
	
	for(int measurement = 0; measurement < numberMeasurements; measurement++){
		printf("%s;%d;%d;",url,ipVersion,measurement);
		for(int tlsVersion=2; tlsVersion<=3;tlsVersion++){
